...
export const updateTicket = async (id: string, body: Partial<Ticket>, token: string): Promise<Ticket> => {
    const data = await axios.patch(`${api_base_url}/tickets/${id}`, body, {
        headers: { Authorization: `Bearer ${token}` }
    });
    return data.data;
}

export const assignTo = (id: string, assigned_to: number, token: string) =>
    updateTicket(id, { assigned_to }, token);

export const changeTicketStatus = (id: string, status_id: number, token: string) =>
    updateTicket(id, { status_id }, token);

export const changeTicketSPriorities = (id: string, priority_id: number, token: string) =>
    updateTicket(id, { priority_id }, token);
